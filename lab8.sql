...
--2 indexes (emo_id and emp_salary_idx)
...
--the index supports queries filtering by dept_id plus salary,
--but not those filtering only by salary
...
--yes, to quickly locate data by leftmost prefix rule
...
--helps to avoid a separate sort step for queries ordering by
...
--would perform a full table scan and apply the LOWER() function
--to each row’s emp_name column to do a case-insensitive search
...
FROM projects
WHERE budget > 80000;
--indexes only a subset of rows that meet a specified condition,
-- making it smaller and more efficient
